...
const FitsPage = () => {
  ...
  // useEffect para INICIALIZAR la instancia de FITS
  useEffect(() => {
  ...
  // useEffect para CARGAR el archivo por defecto
  useEffect(() => {
    if (fitsInstance) {
  ...
  const showHeader = () => {
    if (fitsInstance) fitsInstance.showHeader(true);
  };
  ...
  const formatStats = (roi, title) => {
    ...
    return `${title}: ${pad(roi.width, 4)} x ${pad(roi.height, 5)} @ [${pad(
      fitsCoords.x,
      5
    )}, ${pad(fitsCoords.y, 5)}]
Min ${pad(stats.min.toFixed(1), 11)}    @ [${pad(
      minatCoords.x.toFixed(0),
      5
    )}, ${pad(minatCoords.y.toFixed(0), 5)}]
Max ${pad(stats.max.toFixed(1), 11)}    @ [${pad(
      maxatCoords.x.toFixed(0),
      5
    )}, ${pad(maxatCoords.y.toFixed(0), 5)}]
Mean ${pad(stats.mean.toFixed(1), 10)}  StdDev ${pad(
...
Median ${pad(stats.median.toFixed(1), 8)} Sum ${pad(stats.sum.toFixed(1), 15)}`;
  ...
  const pad = (s, n) => {
    s = s.toString();
    const nadd = n - s.length;
    return ' '.repeat(Math.max(0, nadd)) + s;
  };

  const handleContrastChange = (index, value) => {
    if (!fitsInstance) return;
    setContrasts((prev) => {
      const newContrasts = [...prev];
      newContrasts[index] = value;
      return newContrasts;
    });
  ...
  return (
    <>
      ...
      <Grid templateColumns="320px minmax(600px, 1fr) 320px" gap={4}>
        {/* Columna izquierda - Controles responsive */}
        <GridItem w="full">
          <VStack
            w="full"
            spacing={4}
          ...
          >
            ...
            {/* Canvas del histograma principal */}
            <Box w="full" p={0}>
              <canvas
        ...
        {/* Columna derecha - ROIs responsive */}
        <GridItem w="full">
